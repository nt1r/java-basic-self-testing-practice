...
class GenericTest {
  ...
  private static <T extends Comparable<T>> T min(T[] values) {
    ...
    for (T value : values) {
      if (init) {
          ...
          minValue = minValue.compareTo(value) < 0 ? minValue : value;
      }
    }
  ...
  private static <T> void swapWithinPair(Pair<T> pair) {
    T first_pre = pair.getFirst();
    T second_pre = pair.getSecond();
    pair.setFirst(second_pre);
    pair.setSecond(first_pre);
  }
  // --end-->
}
